...
> `difficulty`: опционально, по умолчанию `medium`. Влияет на инструкцию по сложности внутри промпта:
> - `easy` — общеизвестные факты, доступные без специальных знаний по теме
> - `medium` — вопросы для тех, кто знаком с темой на уровне любителя
> - `hard` — углублённые вопросы, требующие специализированных знаний в области
>
...
| Поле | Описание |
|------|----------|
| `message` | Человекочитаемый итог хода (см. таблицу ниже) |
...
### Клиент → Сервер (индикатор набора)

#### `chat:typing`

> Уведомить участников комнаты/игры, что пользователь печатает
...
> Сервер throttle-ит рассылку: один пользователь может инициировать broadcast не чаще 1 раза в 2 секунды. Ответа от сервера нет; при ошибке приходит `chat:error`.

---

### Сервер → Клиент (индикатор набора)

#### `chat:typing`

> Кто-то из участников печатает сообщение
...
> Клиент отображает индикатор «печатает» и скрывает его через ~3 секунды при отсутствии новых событий.
...
| `role` | ENUM(user, admin) | Роль пользователя (по умолчанию `user`) |
| `status` | ENUM(active, blocked) | Статус аккаунта (по умолчанию `active`) |
| `blocked_at` | BIGINT? | Дата блокировки (null если не заблокирован) |
...
> - `active` — аккаунт активен, пользователь может входить
> - `blocked` — аккаунт заблокирован, вход запрещён (401 при попытке авторизации)
>
> **Роли:**
> - `user` — обычный пользователь
> - `admin` — администратор, имеет доступ к `/admin/*` эндпоинтам
...
|------|---------|----------|
| `role` | `user`, `admin` | Роль пользователя. Назначается вручную через БД. |
| `status` | `active`, `blocked` | Статус аккаунта. `blocked` — вход запрещён. |

> **Назначение роли admin:** изменяется напрямую в БД через SQL или Prisma Studio. Через API роль не назначается.

## HTTP API (требует роль `admin`)

### GET `/admin/users`
Список пользователей с поиском по имени/email и пагинацией.

**Query:** `q`, `page` (по умолчанию 1), `size` (по умолчанию 20, макс. 100)

**Response:** массив пользователей + `pagination.total_elements`

---

### GET `/admin/users/:user_id`
Детальная информация о пользователе (со статистикой).

---

### PATCH `/admin/users/:user_id`
Редактирование пользователя (имя, email). Нельзя изменить роль через API.

---

### DELETE `/admin/users/:user_id`
Удаление пользователя. Нельзя удалить собственный аккаунт.
...
**Body:** `{ status: 'blocked' | 'active', reason?: string }`

> При блокировке все активные refresh-токены продолжают работать до истечения, но JWT strategy отклоняет каждый запрос заблокированного пользователя с `401`.

---

### GET `/admin/reports`
Список жалоб с поиском по имени нарушителя и фильтром по статусу.

**Query:** `q` (поиск по имени нарушителя), `status` (`pending` | `resolved`), `page`, `size`

---

### PATCH `/admin/reports/:report_id/resolve`
Обработка жалобы. Опционально блокирует нарушителя одновременно.

**Body:** `{ comment?: string, block_user?: boolean, block_reason?: string }`
